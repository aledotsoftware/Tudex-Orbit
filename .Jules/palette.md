...
## 2024-05-24 - Restoring Keyboard Accessibility in Frutiger Aero Design
**Learning:** Frutiger Aero and Glassmorphism themes often aggressively reset outlines (`outline: none`) on interactive elements like `.btn` and inputs to maintain a specific visual aesthetic, breaking keyboard accessibility (tab navigation) for visually impaired or power users.
**Action:** Always supplement `outline: none` with an explicit `:focus-visible` pseudo-class using thematic colors and `outline-offset` to restore accessibility without compromising the default un-focused visual design. Furthermore, isolated search inputs must include explicit `aria-label` attributes to be perceivable by screen readers.
