...
## 2024-07-03 - Accessibility: Focus Visible and ARIA Labels
**Learning:** In Frutiger Aero / Glassmorphism UI styles where `outline: none` is often used to maintain aesthetics, keyboard navigation is completely broken unless `:focus-visible` is explicitly added. Furthermore, isolated inputs need `aria-label` since `placeholder` is insufficient for assistive technologies.
**Action:** Always complement `outline: none` with a `:focus-visible` outline utilizing `outline-offset` to preserve both accessibility and design. Ensure inputs without `<label>` tags receive an explicit `aria-label`.
