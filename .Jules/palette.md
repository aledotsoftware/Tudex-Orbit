...
## 2024-06-19 - Accessibility: Focus visibility and ARIA labels in glassmorphic UI
**Learning:** Glassmorphism UI patterns often strip out native focus rings (`outline: none`) to maintain a clean aesthetic, inadvertently making the interface impossible to navigate for keyboard users. Furthermore, isolated inputs (like a top-bar search) lack `<label>` tags and rely on placeholders, which can disappear or not be read by screen readers reliably.
**Action:** Always complement `outline: none` with a `:focus-visible` rule using the theme's accent color (e.g., `#64FFDA`) and `outline-offset` to preserve the visual design while restoring keyboard accessibility. Additionally, ensure isolated inputs have an explicit `aria-label` attribute.
