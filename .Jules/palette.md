...
## 2024-06-17 - Restoring keyboard accessibility in Glassmorphism UIs
**Learning:** In the GeoCore Map Engine repository, which utilizes Frutiger Aero / Glassmorphism UI styles, inputs and buttons often have `outline: none` to fit the theme, which destroys keyboard accessibility. Furthermore, standalone inputs like the search bar may lack associated labels.
**Action:** Always explicitly add a complementary `:focus-visible` rule (e.g., `outline: 2px solid #64FFDA; outline-offset: 2px;`) to restore keyboard navigation without breaking the thematic design, and ensure isolated inputs have an explicit `aria-label` attribute.
