...
## 2024-06-20 - Restore Keyboard Navigation in Glassmorphism UI
**Learning:** In the GeoCore Map Engine, the Frutiger Aero / Glassmorphism UI styles often rely on `outline: none` for aesthetic purposes. This unfortunately breaks keyboard navigation (tabbing) because it removes the default focus rings. Additionally, standalone input fields like the search bar lack `aria-label`s, breaking accessibility for screen readers.
**Action:** Always add explicit `:focus-visible` CSS rules using `outline-offset` to restore keyboard focus rings without breaking the thematic design, and ensure isolated inputs are given an `aria-label`.
