...
## 2024-05-24 - Add focus visible state to inputs and buttons
**Learning:** In the GeoCore Map Engine repository, the Frutiger Aero / Glassmorphism UI styles applied `outline: none` to buttons and inputs. This removed the focus ring, breaking keyboard accessibility.
**Action:** Always explicitly add a complementary `:focus-visible` rule (e.g., `outline: 2px solid #64FFDA; outline-offset: 2px;`) when default focus rings are overridden, and ensure isolated inputs have an explicit `aria-label` attribute.
