...
## 2024-05-24 - Restoring Focus in Frutiger Aero UIs
**Learning:** Frutiger Aero and Glassmorphism UI styles often remove default focus rings (`outline: none`) for aesthetic reasons, which severely degrades keyboard accessibility. Furthermore, isolated inputs (like a search bar) often lack semantic labels.
**Action:** Always verify if `outline: none` is present on interactive elements in these UI themes and explicitly restore them using `:focus-visible` with a complementary color (e.g., `#64FFDA`) and an `outline-offset`. Always add an `aria-label` to isolated inputs that do not have a visible `<label>`.
