...
## 2024-05-24 - Focus visible and isolated inputs in Frutiger Aero UI
**Learning:** Glassmorphism/Frutiger Aero designs often remove outlines (`outline: none`), making keyboard navigation impossible. Isolated inputs without `<label>` elements are inaccessible to screen readers.
**Action:** Always add `aria-label` to isolated inputs and use `:focus-visible` with `outline` and `outline-offset` to restore keyboard accessibility while preserving the thematic design.
