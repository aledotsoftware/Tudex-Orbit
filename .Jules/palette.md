...
## 2024-06-26 - Keyboard Navigation in Glassmorphism UI
**Learning:** Glassmorphism and Frutiger Aero UI styles often remove default focus outlines (`outline: none`) for aesthetic reasons, which severely breaks keyboard accessibility.
**Action:** Always complement `outline: none` with a `:focus-visible` rule utilizing `outline-offset` to restore keyboard navigation without compromising the thematic design, and ensure isolated search inputs are properly labeled for screen readers.
