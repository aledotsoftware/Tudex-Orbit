...
## 2024-06-30 - Restoring Keyboard Focus in Frutiger Aero UI
**Learning:** The Frutiger Aero / Glassmorphism UI styles in GeoCore Map Engine frequently apply `outline: none` to buttons and inputs, which breaks keyboard navigation accessibility.
**Action:** Always explicitly add a complementary `:focus-visible` rule (e.g., `outline: 2px solid #64FFDA; outline-offset: 2px;`) to these elements to restore keyboard navigation without breaking the thematic design, and ensure isolated inputs have an explicit `aria-label` attribute.
