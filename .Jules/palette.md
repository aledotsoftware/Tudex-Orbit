...
## 2024-05-15 - Accessibility: Restoring keyboard navigation on custom inputs and buttons
**Learning:** In the GeoCore Map Engine repository, which uses a glassmorphism UI style, elements like `.btn` and `#search-input` have `outline: none;` which breaks keyboard navigation. Furthermore, the search input lacks an `aria-label`.
**Action:** Always add a complementary `:focus-visible` rule (e.g., `outline: 2px solid #64FFDA; outline-offset: 2px;`) to restore keyboard navigation without breaking the thematic design, and ensure isolated inputs have an explicit `aria-label` attribute.
